...
export function CommitGraph({ data }: CommitGraphProps) {
    ...
    // SVG dimensions
    const svgWidth =
        LABEL_OFFSET + totalWeeks * (CELL_SIZE + CELL_GAP) + CELL_GAP;
    const svgHeight =
        MONTH_LABEL_HEIGHT + 7 * (CELL_SIZE + CELL_GAP) + CELL_GAP + 30; // +30 for legend
    ...
    const handleMouseEnter = useCallback(
        (cell: CellPosition, event: React.MouseEvent<SVGRectElement>) => {
            ...
            const cellSvgX =
                LABEL_OFFSET +
                cell.weekIndex * (CELL_SIZE + CELL_GAP) +
                CELL_SIZE / 2;
            const cellSvgY =
                MONTH_LABEL_HEIGHT + cell.dayIndex * (CELL_SIZE + CELL_GAP);
                ...
                >
                    ...
                    {cells.map((cell) => (
                        <rect
                            ...
                            y={
                                MONTH_LABEL_HEIGHT +
                                cell.dayIndex * (CELL_SIZE + CELL_GAP)
                            }
                            width={CELL_SIZE}
                            height={CELL_SIZE}
                            rx={CELL_RADIUS}

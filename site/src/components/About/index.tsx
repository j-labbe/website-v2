...
export const About = () => {
    ...
    const totalWordAnimationMs = aboutParagraphLengths.reduce((a, b) => a + b, 0) * WORD_DELAY_MS;
    ...
    useEffect(() => {
        ...
        const timeoutId = window.setTimeout(() => {
            setShowCursor(true);
        }, totalWordAnimationMs);

        return () => window.clearTimeout(timeoutId);
    }, [inView, totalWordAnimationMs]);
    ...
    return (
        <div ref={containerRef} className="mb-8">
            {ABOUT_TEXT.map((paragraph, index) => {
                ...
                return (
                    <p key={index} className={`text-lg leading-relaxed text-text max-w-5xl ${index !== 0 ? "mt-4" : ""}`}>
                        <span>
                            {paragraph.split(" ").filter(Boolean).map((word, i) => {

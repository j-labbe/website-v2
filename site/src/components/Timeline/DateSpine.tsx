                ...
                {items.map((item) => {
                    if (item.type === "year") {
                        return (
                            <li
                                key={`year-${item.year}`}
                                className="flex items-center justify-end py-0.5 cursor-default"
                            >
                    ...
                    const { scale, brightness } = getMagnification(
                        item.monthIndex,
                        hoveredMonthIndex,
                    );
                    ...
                    const labelOpacity = spineHovered
                        ? 0.3 + brightness * 0.7
                        : 0;
                    const labelColor = isActive
                        ? COLOR_ACCENT
                        : lerpColor("#6e7a8a", COLOR_BRIGHT, brightness);
                    ...
                    return (
                        <li
                            ...
                            onMouseEnter={() =>
                                setHoveredMonthIndex(item.monthIndex)
                            }
                            onClick={() => handleClick(item.key)}
                            role="button"
                            tabIndex={0}
                            onKeyDown={(e) => {
                                if (e.key === "Enter") handleClick(item.key);
                            }}
